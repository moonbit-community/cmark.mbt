...
The setext heading underline cannot be a
[lazy continuation line] in a list item or block quote:
...
or use a thematic break that cannot count as a
[setext heading underline], such as
...
as a code block and as indicating that material belongs to a
[list item][list items], the list item interpretation takes precedence:
...
the first line meets both the [start condition] and the
[end condition], the block will contain just that line.
...
a document, and except for blocks of types 1--5,
[above][HTML block]:
...
that ends with two or more spaces will not end with a
[hard line break]:
...
3.  **Consecutiveness.**  A document cannot contain two
    [block quotes] in a row unless there is a [blank line] between them.
...
The link's URI and title are provided by the matching
